...
<input type="hidden" name="id" value="<#if navi_bar_item??>${navi_bar_item.id?c!""}</#if>" >
</div>
<!--导航栏-->
...
<div class="tab-content" style="display: block;">
	    ...
	    <dd>
	      <div class="rule-single-select single-select">
	        <select name="parentId" style="display: none;">
	            <option value="" <#if navi_bar_item?? && navi_bar_item.parentId?? && navi_bar_item.parentId==0>selected="selected"</#if>>无父级分类</option>
	        	<#if navi_bar_item_list??>
	        	   <#list navi_bar_item_list as c>
	        	       <option value="${c.id?c!""}" <#if navi_bar_item?? && navi_bar_item.parentId?? && navi_bar_item.parentId==c.id>selected="selected"</#if>><#if c.layerCount?? && c.layerCount gt 1><#list 1..(c.layerCount-1) as a>　</#list>├ </#if>${c.title!""}</option>
	        	   </#list>
	        	</#if>
	        </select>
	      </div>
    ...
    <dl>
        <dt>别名</dt>
        <dd>
            <input name="engTitle" type="text" value="<#if navi_bar_item??>${navi_bar_item.engTitle!""}</#if>" class="input normal"  sucmsg=" ">
            <span class="Validform_checktip">英文名称</span>
        </dd>
    </dl>
    <dl>
        <dt>图标</dt>
