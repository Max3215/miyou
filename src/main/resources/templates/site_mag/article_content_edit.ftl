...
<script type="text/javascript">
    $(function () {
        ...
        var logo1Pic = $("#videoUrl").val();
        if (logo1Pic == "" || logo1Pic == null) {
            $(".thumb_videoUrl_show").hide();
        }
        else {
            $(".thumb_videoUrl_show").html("<ul><li><div class='img-box1'><img src='" + logo1Pic + "' bigsrc='" + logo1Pic + "' /></div></li></ul>");
            $(".thumb_videoUrl_show").show();
        }
...
<input name="id" type="text" value='<#if article??>${article.id?c!""}</#if>' style="display:none">
    ...
    <div class="tab-content" style="display: block;">
        ...
        <dl>
            <dt>视频</dt>
            <dd>
                <input id="videoUrl" name="videoUrl" type="text" datatype="*0-255" value="<#if article??>${article.videoUrl!""}</#if>" class="input normal upload-path">
                <div class="upload-box upload-img"></div>
                <span class="Validform_checktip"></span>
                <div class="photo-list thumb_videoUrl_show"></div>
            </dd>
        </dl>
        <dl>
            <dt>排序数字</dt>
